<div class="site-block text-center">
    <div class="btn-group portfolio-filter">
        <a href="{{ route('nav', [$parentLink]) }}" class="btn btn-primary active" data-category="all">{{ __('All') }}</a>
        @foreach( $subCategories as $subCategory)
        <a href="{{ route('sub_Nav', [$parentLink, $subCategory->link]) }}" id="linkToSubcate" class="btn btn-primary" data-category="design">{{ $subCategory->name }}</a>
